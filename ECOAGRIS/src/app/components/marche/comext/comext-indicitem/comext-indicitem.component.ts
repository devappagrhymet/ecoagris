...
@Component({
  ...
  templateUrl: './comext-indicitem.component.html',
  styleUrl: './comext-indicitem.component.scss',
  animations: [
    trigger('listItem', [
      state('default', style({
        transform: 'scale(1)',
...
export class ComextIndicitemComponent {
  ...
  ngAfterViewInit() {
   ...
   ngOnInit(): void {
    ...
    this._getData_Indicator_Pays(2021,12,83);
      /*===================*/
  ...
  getData_Indicator_Pays(params_campagne,params_pays,params_indicateur): Observable<any> {
    ...
    let fin_campagne = params_campagne;
...
/*
  __old__getData_Indicator_Pays(params_campagne,params_pays,params_indicateur): Observable<any> {
     let debut_campagne =  params_campagne;
    let fin_campagne = params_campagne + 1;
    let pays = params_pays;
    let indic = params_indicateur

    return this._http.get('http://154.127.90.218:8000/api/marche/getComextByPays/'+debut_campagne+'-'+fin_campagne+'-'+pays+'-'+indic);
